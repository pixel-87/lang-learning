...
Go was designed to be concurrent, which is a traint *fairly* unique to Go. It excels at performing many tasks simultaneously safely using a simple syntax.
...
At Textio we send *a lot* of network requests. Each email we send must go out over the internet. To server our millions of customers, we need a single Go program to be capable of sending *thousands of emails* at once.

Edit the `sendEmail()` function to execute its ananymous function concurrency so that the "received" message prints *after* the "sent" message.
